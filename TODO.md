...
- [x] Code tab: renders the draft request as a runnable command — `internal/codegen` (curl one-liner, curl-as-bash-script, PowerShell Invoke-RestMethod one-liner, PowerShell script), mirroring `httpengine.Execute`'s substitution/headers/auth/body handling. `POST /api/codegen`, `selectCodeFormat`/`copyRequestCode` ui:actions, `codeFormat`/`code` in `GET /api/ui/state`, Go + control-API test coverage. Shipped 2026-09-05.
