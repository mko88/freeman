...
directory. Run it back-to-back as many times as you like. Two
exceptions: scripts/random-sample.bin, a committed random-bytes fixture
the file-upload test only ever reads (reused across runs, not
regenerated or deleted); and the response cache under
<appdata>/freeman/responses, which test_response_cache clears whole as
part of what it tests — a disposable convenience cache (the settings
window has the same button), regenerated by re-sending any request.
Switching to a brand-new, empty workspace like
...
def test_response_cache(api: ControlAPI, r: Report, collection_id: str, environment_id: str, item_id: str) -> None:
    ...
    Note: clearResponseCache wipes <appdata>/freeman/responses whole,
    including any real cached responses the user has — it's a disposable
    convenience cache (the settings window offers the same button), so
    this is an accepted exception to the "leaves no trace" rule above,
    like scripts/random-sample.bin is. Everything else this test creates
    is still cleaned up (its scratch request, and its cache entry with
    it, in main()'s Phase 3)."""
